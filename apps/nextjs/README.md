...
Currently we have small end-to-end test suite.

It checks that:

- an unauthenticated user is redirected from /chat to /sign-in
- an authenticated user can reach the /chat page
- an authenticated user can

To run the tests with the Playwright UI, run:

```sh
pnpm run test-e2e-ui
```

from within `/apps/nextjs`.
