...
export function DebugOverlay() {
  ...
  const modelRows = (() => {
    ...
    const { chat, moderation, categorise, agentic } = overlayState.data.models;
    const entries: { label: string; value: string }[] = [
      { label: "Chat", value: chat },
    ];
    if (moderation !== chat)
      entries.push({ label: "Moderation", value: moderation });
    if (categorise !== chat)
      entries.push({ label: "Categorise", value: categorise });
