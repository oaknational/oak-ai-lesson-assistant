...
export type LockingModerationViewProps = Readonly<{
  chatId: string;
...
const LockingModerationView = ({
  chatId,
  moderation,
  heading,
  body,
}: LockingModerationViewProps) => {
    ...
    <div className="fixed inset-0 z-50 flex w-full items-center justify-center bg-lavender30">
      <div className="m-18 max-w-[600px] flex-col gap-13 border-2 border-gray-900 bg-white p-18">
        ...
        <div className="flex w-full flex-col gap-11 text-base">
          <div>
            <h2 className="mb-6 mt-12 text-lg font-bold">{heading}</h2>
            <p>{body}</p>
          </div>
          {hasSubmitted ? (
            <p>
...
export default LockingModerationView;
