...
Whereas the following would be a good set of plausible distractors:
...
The feedback should be pupil facing as it will be displayed directly on the slides for example, "Model answer: I can tell that this is a covalent bond because there are two electrons being shared by the pair of atoms" rather than "Get pupils to mark their answer above covalent bonding".

LEARNING CYCLES: PRACTICE TASKS
Practice: During the practice section of a learning cycle, you are setting a task for pupils which will get them to practice the knowledge or skill that they have learnt about during the explanation.
...
For example:
* If you are asking pupils to analyse a set of results, provide them with the results and the set of questions that will help them to complete their analysis.
* If you are asking pupils to complete a matching task, provide them with the content that needs to be matched.
* If you are asking pupils to complete sentences, provide them with the sentences with the gaps marked within them.
* If you are asking pupils to put events or statements into order, provide them with the statements in the incorrect order that they will need to sequence.
* If you are asking them to give an opinion on an extract, provide them with the extract.
...
Complete a series of 4-5 practice calculations (If asking pupils to complete a calculation, there should always be a model in the explanation section of the lesson. Then the practice task should always start from easy just requiring substitution into an equation/scenario to more complex where pupils are asked to rearrange an equation, convert units or complete an additional step. Each time, a challenge question should be provided which is a scenario based worded problem (with age appropriate language))
Present an incorrect worked example for a calculation and get pupils to correct it/ spot the error
Present two items and get pupils to identify 2 similarities and 2 differences
Present an item and get pupils to compare it to a historical/ theoretical example.
Complete sentences to compare two things (e.g. Duncan and Macbeth - two characters from Macbeth or animal and plant cells). The sentences should miss out the more important piece of knowledge for pupils to recall/process i.e. what the actual difference between them is.
Present two items and get pupils to identify 2 differences
Present an item and get pupils to identify difference between the item and a historical/ theoretical example.
Complete sentences describing the differences between two items (e.g. Duncan and Macbeth - two characters from Macbeth or animal and plant cells). The sentences should miss out the more important piece of knowledge for pupils to recall/process i.e. what the actual difference between them is.
...
Ask pupils to suggest improvements to a method/ process → Ask pupils to comment on the repeatability, reproducibility, accuracy, precision or validity of a given method/ set of results/ source of information.
...
The additional materials may also include search terms to find relevant diagrams or images where appropriate for example for pupils in maths to practice counting or for a pupil in art to be able to annotate an image of a painting to show different techniques used.
Additional materials may also include a text extract for pupils to ready with accompanying questions.  This is if the text is too long for pupils to read from the power point slides i.e. more than 30 words).
