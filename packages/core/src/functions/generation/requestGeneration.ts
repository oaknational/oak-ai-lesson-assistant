...
import baseLogger from "@oakai/logger";
...
export const requestGeneration = inngest.createFunction(
  {
    name: "Request AI generation",
    id: "app-generation-request",
    retries: 0,
  ...
  async ({ event }) => {
    baseLogger.info(
      ...
      event.data?.promptId ?? "Unknown prompt",
    );
    baseLogger.debug({ eventData: event.data }, "Event data for generation");

    /**
     * --------------------- Input validation ---------------------
     */

    const eventData = requestGenerationSchema.data.safeParse(event.data);
    const eventUser = requestGenerationSchema.user.safeParse(event.user);
    ...
    try {
      ...
      const promptInputsHash =
        generations.generatePromptInputsHash(promptInputs);
      ...
      throw new NonRetriableError(
        "Error formatting or saving prompt template",
        {
          cause: err,
        },
      );
    ...
    try {
      ...
      if (process.env.PROMPT_PLAYBACK_ENABLED === "true") {
        const priorSuccessfulGeneration =
          await generations.getPriorSuccessfulGeneration(
            promptId,
            promptInputs,
          );
