...
import type { TestModelOptions } from "../types/testTypes";
  ...
  describe("Model Detection and Parameter Routing", () => {
    ...
    it("should apply correct parameter branching logic", () => {
      ...
      testCases.forEach((testCase) => {
        ...
        const options: ModelOptions = {
          ...
          reasoning_effort: testCase.reasoning_effort as
            | ReasoningEffort
            | undefined,
    ...
    it("should handle missing parameters gracefully", () => {
      ...
      testCases.forEach((testCase) => {
        const options: ModelOptions = {
          temperature:
            "temperature" in testCase
              ? (testCase.temperature as number)
              : undefined,
          reasoning_effort:
            "reasoning_effort" in testCase
              ? (testCase.reasoning_effort as ReasoningEffort)
              : undefined,
          verbosity:
            "verbosity" in testCase
              ? (testCase.verbosity as Verbosity)
              : undefined,
        };
        ...
        if (typedParams.type === "gpt5") {
          expect(typedParams.reasoning_effort).toBe(
            testCase.expectReasoningEffort,
          );
    ...
    it("should demonstrate parameter precedence", () => {
      ...
      const gpt5Params = createModelParams(
        gpt5Model,
        [],
        mixedOptions,
        isGPT5Model,
      ...
      const legacyParams = createModelParams(
        legacyModel,
        [],
        mixedOptions,
        isGPT5Model,
