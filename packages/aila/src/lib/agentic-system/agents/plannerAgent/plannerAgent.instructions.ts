...
- \`ADD_QUIZ_QUESTION\` — "add a question about X", "add another one"
- \`CHANGE_QUIZ_QUESTION\` — "change question N", "rewrite question 2", "make question N easier"
- \`REGENERATE_QUIZ\` — "generate a new quiz", "redo the whole quiz"
...
**Empty-quiz exception:** When the classified action would be \`ADD_QUIZ_QUESTION\` and the target quiz has no questions in the current document, emit \`REGENERATE_QUIZ\` instead — adding a single question to an empty quiz is meaningless; the user wants a quiz generated.
