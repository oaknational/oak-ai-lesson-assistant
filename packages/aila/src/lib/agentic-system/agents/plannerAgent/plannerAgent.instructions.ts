...
When planning a step for a list-based section — \`starterQuiz\`, \`exitQuiz\`, \`keywords\` or \`misconceptions\` — check whether the user message expresses a structured single-item modification request and, if so, emit an \`itemIntent\` on that step. The "items" are the quiz's questions, the individual keywords, or the individual misconceptions.

**Action classification:**
Classify the user message into one of these actions:
- \`REMOVE_ITEM\` — "remove question N", "delete the third question", "remove the photosynthesis keyword", "drop the osmosis misconception"
- \`ADD_ITEM\` — "add a question about X", "add another keyword", "add a misconception". **Before emitting this, check the target section in the CURRENT DOCUMENT — if it is empty or absent, emit \`REGENERATE_SECTION\` instead (see Empty-section exception below).**
- \`CHANGE_ITEM\` — "change question N", "rewrite the second keyword", "make question N easier", "reword the osmosis misconception"
- \`REGENERATE_SECTION\` — "generate a new quiz", "redo the keywords", "start the misconceptions again", **or any \`add\`-shaped request when the target section has zero existing items**

The step's \`sectionKey\` should be the target section (\`starterQuiz\`, \`exitQuiz\`, \`keywords\` or \`misconceptions\`).

**Position resolution:**
For \`REMOVE_ITEM\` and \`CHANGE_ITEM\`, resolve \`position\` (1-indexed) from the message:
- Explicit number: "remove question 3" → \`position: 3\`
- Ordinal: "remove the first keyword" → \`position: 1\`, "remove the last question" → \`position: <number of items in the section>\`
- Content reference: "remove the photosynthesis keyword" → resolve to the matching item's 1-indexed position
- If position cannot be determined, emit \`position: null\`

For \`ADD_ITEM\`, emit the requested insert position if given, otherwise \`position: null\` (append).
For \`REGENERATE_SECTION\`, emit \`position: null\`.

**Empty-section exception (mandatory check before classifying ADD):**
Before emitting \`ADD_ITEM\`, count the target section's existing items in the CURRENT DOCUMENT:
- For \`starterQuiz\`/\`exitQuiz\`, count \`questions\` (e.g. \`starterQuiz.questions\`) — the section is an object wrapping that array.
- For \`keywords\`/\`misconceptions\`, count the array itself.
If the section has zero items (or is absent), you MUST emit \`REGENERATE_SECTION\` instead of \`ADD_ITEM\`, with \`position: null\`. Adding a single item to an empty section is invalid — the user is asking for the section to be generated, not for one item appended to nothing.

Example:
- User message: "For the starter quiz, add a question"
- Document: \`starterQuiz: { questions: [], ... }\`
- Correct output: \`action: REGENERATE_SECTION\`, \`position: null\` (NOT \`ADD_ITEM\`)

If the message is not a single-item modification request for a list-based section, omit \`itemIntent\` (or set it to \`null\`) on the step.
