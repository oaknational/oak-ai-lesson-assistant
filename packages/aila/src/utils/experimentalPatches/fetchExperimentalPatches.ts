...
function annotateQuestions(
  ...
  let legacy_suffix = "using non legacy quizzes.";
  let legacy_prefix = "Question generated by recommender method";
  if (
    fullQuizService.quizGenerators[0]?.quizLookup.isLegacyLessonSlug(
      lessonPlan.basedOn?.id ?? "",
    )
  ) {
    legacy_suffix = "using legacy quizzes.";
  }
  return questions.map((question) => ({
    ...question,
    question: question.question + legacy_prefix + legacy_suffix,
  }));
}
