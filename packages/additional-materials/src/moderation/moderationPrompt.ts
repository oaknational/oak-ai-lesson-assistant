import moderationCategories from "@oakai/core/src/utils/ailaModeration/moderationCategories.json";
...
export function generateAdditionalMaterialModerationPrompt(categories: ModerationCategory[]): string {
...
- 1: ${cat.criteria1}
  ...
  return `OBJECTIVE:
You are a content moderation supervisor examining an additional material resource document which has been generated by a user through interactions with Oak National Academy's additional material assistant. Your job is to:
1. Assess the additional material resource against a list of specific content categories detailed below.
...
*   Your primary goal is to provide content guidance to ensure that certain content can be flagged with an 'adult supervision recommended/required' label. The score for each category must reflect the presence and significance of relevant content.
...
*   For all other categories, when determining a score, consider the Key Stage (age group) of the pupils if Key Stage is specified within the additional material resource.
*   Regardless of the educational context, the score must be adjusted to reflect the presence of such content, even if the content is presented in a careful, considerate manner.
...
export const moderationPrompt = generateAdditionalMaterialModerationPrompt(moderationCategories as ModerationCategory[]);
