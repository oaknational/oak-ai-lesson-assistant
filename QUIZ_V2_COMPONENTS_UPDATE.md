...
- **Note**: Kalam is a Google Font with handwriting style that Oak uses for resources (PDFs, exports, etc.)
- It is NOT used on the main website/application
- Current implementation uses `body-2-bold` which is appropriate for web display
- If we need to match resource styling exactly, we would need to import and use Kalam font

### Markdown Component Customization

- **Note**: Currently using CSS to style `<strong>` elements in ShortAnswerQuestion inline answers
- Consider allowing custom components for markdown elements (e.g., custom `<strong>` component)
- This would enable better ARIA support and semantic markup for answer elements
- Could use react-markdown's `components` prop to override default elements
